  ...
  <table>
    ...
    <?php foreach ($widgetSymbols as $symbol): ?>
      ...
      <?php foreach ($widgetFields as $code):?>
        ...
        <?php else:?>
        <td data-title="<?php print $liveDataFields[$code]->name?>"><span class="smw-market-data-field <?php print in_array($code,['quote.regularMarketChange','quote.regularMarketChangePercent'])?'smw-change-indicator':''?>" data-symbol="<?php print $symbol ?>" data-field="<?php print $code?>"></span></td>
        <?php endif?>
        <?php endforeach?>
      </tr>
    <?php endforeach;?>
