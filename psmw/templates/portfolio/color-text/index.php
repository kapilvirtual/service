...
<div id="<?php print $widgetId;?>" class="<?php print $widgetClass?>" <?php print $widgetDataAttributes?>>
  <table>
    ...
    <tfoot>
      <tr>
        <?php foreach ($widgetFields as $code):?>
          <td class="smw-portfolio-total-<?php print str_replace('.','-',$code)?>" colspan="<?php print $code=='quote.regularMarketPrice'?2:1?>" data-title="<?php print $liveDataFields[$code]->name?>"></td>
        <?php endforeach?>
      </tr>
    </tfoot>
  </table>
    ...
    $(document).ready(function() {
      ...
      $widget.on('psmwReadyGlobal', function (event) {
        ...
        if (assetsCount>0) {
          ...
          $widget.find('.smw-portfolio-total-quote-regularMarketChange').text(avgAbsChange.formatNumber());
          $widget.find('.smw-portfolio-total-quote-regularMarketChangePercent').text(avgPctChange.formatNumber()+'%');
        }
