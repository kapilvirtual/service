...
var premiumStockMarketWidgetsPlugin = (function ($) {
  ...
  function initializeWidgets(retrieveMarketDataFromServer) {
    ...
    $document
      .off('psmwReadyGlobal', classWidgets)
    ...
    $document
      ...
      .on('psmwReadyGlobal', classWidgets, makeWidgetVisible)
      // hide modal
  ...
  function addDataToWidgets($widgetContainers, marketData) {
    ...
    $widgetContainers.each(function(i, widgetContainer) {
      ...
      $widgetContainer.find(classMarketDataField).each(function(k, widgetField) {
        ...
        if (['virtual.symbolLink','virtual.nameLink'].indexOf(marketDataFieldCode) === -1) {
          $widgetField.text(marketDataFieldValue);
        } else {
          $widgetField.html(marketDataFieldValue);
        }
  ...
  /**
   * Values comparison function for tablesort plugin
   * @param s1
   * @param s2
   * @returns {number}
   */
  function tablesortCompare(s1, s2) {
    var n1 = parseFloat(s1.replace(/[^0-9.-]/g,''));
    var n2 = parseFloat(s2.replace(/[^0-9.-]/g,''));
    return s1 && s2 && !isNaN(n1) && !isNaN(n2) ? (n1>n2?1:(n1<n2?-1:0)) : s1.localeCompare(s2);
  }
  ...
  return {
    ...
    tablesortCompare:   tablesortCompare,
    checkIE:            checkIE,
    log:                log
  };
})(jQuery);